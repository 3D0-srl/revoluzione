...
class Marion{
            ...
            foreach($actions_sorted as $function)
            {
				...
				if( preg_match('/::/',$function['function']) ){
					...
					if( class_exists($class_name) ){
						$obj = new $class_name();
						if (is_array($params) ){
							call_user_func_array([$obj,$func],$params);
						}else{
							call_user_func([$obj,$func],$params);
						}
	...
	*/
	...
	public static function loadModules(){
		$db = self::getDB();
		self::$modules = $db->select('directory,scope,active,autoload','module',"1=1");
		foreach(self::$modules as $mod){
			if( $mod['autoload'] && file_exists(_MARION_MODULE_DIR_.$mod['directory']."/vendor/autoload.php")){
