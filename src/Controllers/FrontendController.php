...
class FrontendController extends Controller{
	...
	function notLogged(){
		header('Location: '._MARION_BASE_URL_."index.php?ctrl=Access&action=login");
		exit;
	}
	...
	function setMedia(){
		$this->registerCSS('plugins/bootstrap/bootstrap.min.css');
		$this->registerCSS('assets/flag-icon-css/css/flag-icon.css');
		$this->registerCSS('plugins/sweetalert/sweetalert.css');
		$this->registerCSS('plugins/magnific/magnific-popup.css');

		$this->registerJS('plugins/jquery-1.11.js','head',10);
		$this->registerJS('plugins/bootstrap/bootstrap.min.js','head',11);

		$this->registerJS('plugins/sweetalert/sweetalert.min.js','head');
		$this->registerJS('plugins/magnific/jquery.magnific-popup.min.js','head');
	...
	function initTwingTemplate(){
		...
		if( $this->_module ){
			...
			if( file_exists(_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig") ){
				

				$directories_templates_twig[] =_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig";
				//$loader->addPath(_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig");

				if( file_exists(_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig/front") ){
					$directories_templates_twig[] =_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig/front";
					//$loader->addPath(_MARION_THEME_DIR_._MARION_THEME_."/modules/".$this->_module."/templates_twig/front");
				}
			}

			if( file_exists('modules/'.$this->_module."/templates_twig") ){

				if( file_exists('modules/'.$this->_module."/templates_twig/front")){
					//$loader->addPath('modules/'.$this->_module."/templates_twig/front");
					$directories_templates_twig[] = 'modules/'.$this->_module."/templates_twig/front";
				}
				$directories_templates_twig[] = 'modules/'.$this->_module."/templates_twig";
				//$loader->addPath('modules/'.$this->_module."/templates_twig");
			}
		}
		//debugga($directories_templates_twig);exit;
		...
		$twig_functions[] =new \Twig\TwigFunction('cartEmpty', function () {
			...
			if( !$GLOBALS['cart_number_product'] ){
				if( authUser()){
					...
					$cont = $database->select('sum(quantity) as sum','cartRow',"cart={$cart->id} AND (parent is NULL OR parent=0)");
					$GLOBALS['cart_number_product'] = $cont[0]['sum'];
				}else{
					$num = Cart::getCurrentNumberProduct();
					$GLOBALS['cart_number_product'] = $num;
	...
	function cartNumberProduct(){
		if( !array_key_exists('cart_number_product',$GLOBALS) ){
			if( authUser()){
				$database = _obj('Database');
	...
	function  switchLocale(){
        if (okArray(getConfig('locale','supportati'))) {
