...
class Db{
	...
	public function insert($name,$content,$html,$UserId) {
	
    try {
      $conn=$this->connect();
			$stmt = $conn->prepare("INSERT INTO `bal_email_builder` (`name`,`content`,`UserId`,`html`) VALUES (?, ?, ?,?)");
			$stmt->bind_param("ssss", $name, $content,$UserId,$html);
