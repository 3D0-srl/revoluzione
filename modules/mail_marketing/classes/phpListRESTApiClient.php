...
class phpListRESTApiClient
{
	...
	public function listCampaignAdd($list_id = 0, $campaign_id = 0){
		 $post_params = array(
            'list_id' => $list_id,
     ...
     * @return int ListId of the list created
     */
    public function campaignAdd($post_params)
    {
        // Create minimal params for api call
        $post_params_default = array(
            'subject' => 'oggetto',
            'fromfield' => 'ciro@3d0.it',
            'replyto' => '',
            'message' => '',
			'textmessage' => '',
			'footer' => '',
			...
			'owner' => 1,
			'htmlformatted' => 1,
			'repeatuntil' =>  date('Y-m-d H:i:s',strtotime('+1 hour')),
			'embargo' =>  date('Y-m-d H:i:s'),

        );

		foreach($post_params_default as $k => $v){
			if( !$post_params[$k] ){
				$post_params[$k] = $v;
			}
		}
	...
	public function getConnection(){
		$result = $this->callAPI('getConnection', $post_params);
		debugga($result);exit;
