...
class FormBuilder extends Base{
	...
	/*function getFlexyElements(&$elements=array(),$action=null,$data=NULL,$number=null,$name_array_form=''){
		

		$campi = $this->getFields();


		$elements["action{$number}"] = new HTML_Template_Flexy_Element;
		$elements["action{$number}"]->setValue($action);
		$elements["action{$number}"]->attributes['id'] = "action{$number}";
		

		foreach($campi as $campo){
			if( !$name_array_form ){
				$key_campo = "{$campo->campo}{$number}";
			}else{
				$key_campo = "{$name_array_form}{$number}[{$campo->campo}]";
			}
			...
			$elements[$key_campo] = new HTML_Template_Flexy_Element;
			$elements[$key_campo]->attributes['id'] = $campo->campo;
			if( $type == 'multiselect'){
				$type = 'select';
			}
			$elements[$key_campo]->attributes['type'] = $type;
			if( $type != 'radio'){
				
				if( $campo->hasOptions()){
					$options = $campo->getOptions();
					
					$elements[$key_campo]->setOptions($options);
				}
			}
