...
class SubmitController extends FrontendController
{
	...
	function ajax()
	{
		...
		$data = $database->select('*', 'composition_page', "id={$formdata['form_builder_id']}");
		...
		*/
		...
		foreach ($campi as $row) {
			...
			//if ($row['type'] == 'checkbox' || $row['type'] == 'radio' && $row['mandatory'] == 1) {
			if ($row['type'] != 'file' && $row['type'] != 'checkboxes' && $row['type'] != 'multiselect' && $row['mandatory'] == 1) {
				...
				/*if ($formdata) {
					...
					if (!$flag) {
						$errors[] = "Il campo {$name} è obbligatorio<br>";
					}
				} else {
					$errors[] = "Il campo {$name} è obbligatorio<br>";
				}*/
