...
class PageComposerAdminController extends ModuleController{
	public $_auth = 'cms';
	...
	function display(){
		...
		switch($action){
			...
			case 'export':
				...
				$items = $database->select('*','composition_page',"id_adv_page={$id} order by orderView");
			...
			case 'sort_items':
				...
				foreach($items as $k => $v){
					...
					switch($v['type']){
						case 'row':
							$items[$k]['img_logo'] = '/img/composer/row-blank.png';
							break;
					}
				}
			...
			case 'edit_ok':
				...
				$database->delete('composition_page',"id_adv_page={$id}");
				
				$database->execute("INSERT composition_page SELECT * FROM composition_page_tmp where id_adv_page={$id};");
				$database->delete('composition_page_tmp',"id_adv_page={$id}");
				...
				$js_head = file_get_contents($path.'modules/pagecomposer/media/js/js_head_tmp_'.$id.".js");
				if( $js_head ){
					file_put_contents($path.'modules/pagecomposer/media/js/js_head_'.$id.".js",$js_head);
				}else{
					unlink($path.'modules/pagecomposer/media/js/js_head_'.$id.".js");
				}

				$js_end = file_get_contents($path.'modules/pagecomposer/media/js/js_end_tmp_'.$id.".js");
				if( $js_end ){
					file_put_contents($path.'modules/pagecomposer/media/js/js_end_'.$id.".js",$js_end);
				}else{
					unlink($path.'modules/pagecomposer/media/js/js_end_'.$id.".js");
				}


				
				file_put_contents($path.'modules/pagecomposer/media/css/css_'.$id.".css",file_get_contents($path.'modules/pagecomposer/media/css/css_tmp_'.$id.".css"));

				
				$database->execute("update page_advanced set custom_css=custom_css_tmp,custom_js_head=custom_js_head_tmp,custom_js_end=custom_js_end_tmp where id={$id}");
	...
	function setMedia(){
		...
		$this->registerJS($this->getBaseUrl().'plugins/jquery-ui/jquery-ui-1.10.4.min.js','end');
		$this->registerJS($this->getBaseUrl().'plugins/jquery-nestable/jquery.nestable.js','end');
	...
	function getWidgets(){
		$widgets = array( 
			array(
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-blank.png',
				'icon-active' => '/img/composer/row-blank-active.png',
				'repeat' => 1,
			),
			array(
				'title' => '1 col.',
			...
			array(
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-2.png',
			'icon-active' => '/img/composer/row-2-active.png',
				'repeat' => 1,
			),
			...
			array(
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-3.png',
				'icon-active' => '/img/composer/row-3-active.png',
				'repeat' => 1,
			),
			array(
				'title' => '4 col.',
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-4.png',
				'icon-active' => '/img/composer/row-4-active.png',
				'repeat' => 1,
			),
			...
			array(
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-75-25.png',
				'icon-active' => '/img/composer/row-75-25-active.png',
				'repeat' => 1,
			),
			array(
				'title' => '2 col. 25 / 75',
				...
				'module' => '',
				'id' => 0,
				'icon' => '/img/composer/row-25-75.png',
				'icon-active' => '/img/composer/row-25-75-active.png',
				'repeat' => 1,
			),
			/*array(
	...
	function editor_css_ajax(){
		...
		$database = Marion::getDB();
		$database->update('composition_page_tmp',"id={$id}",$formdata);
	...
	function editor_css(){
		$id = _var('id_box');
		$database = Marion::getDB();
		$dati = $database->select('*','composition_page_tmp',"id={$id}");
		if( okArray($dati) ){
	...
	function editor(){
		...
		$page = Page::prepareQuery()->where('id_adv_page',$id)->getOne();
		...
		if( $id != _PAGE_COMPOSER_DASHBOARD_PAGE_ID_ ){
			...
			if( okArray($_page) ){
				...
				if( okArray($layout) ){
					$_template_vars['layout_name'] = $layout[0]['nome'];
		...
		$_functions_pagecomposer_twig = array(
				...
				new \Twig\TwigFunction('page_composer_buttons', function ($dati) {
					...
					$loader = new \Twig\Loader\FilesystemLoader('../modules/pagecomposer/templates_twig/admin');
	...
	function ajax(){
		...
		switch($action){
			case 'save_custom_name':
				...
				$name = _var('name');
				$database->update('composition_page_tmp',"id={$id_widget}",array('custom_name'=>$name));
				$risposta = array(
					'result' => 'ok',
				);
				echo json_encode($risposta);
			...
			case 'save_css':
				...
				try{
					...
					$parameters = array(
						...
						'THEME_DIR' => $this->getBaseUrl()."themes/",
					);
					$string = '';
					...
					file_put_contents($path.'modules/pagecomposer/media/css/css_tmp_'.$id.".css",$compressed);
				}catch(Exception $e){
				...
				file_put_contents($path.'modules/pagecomposer/media/js/js_head_tmp_'.$id.".js",$js_head);
				file_put_contents($path.'modules/pagecomposer/media/js/js_end_tmp_'.$id.".js",$js_end);
				...
				$database->update('page_advanced',"id={$id}",$toinsert);
			...
			case 'edit_page_ok':
				...
				$path = $_SERVER['DOCUMENT_ROOT']._MARION_ROOT_DIR_;

				
				file_put_contents($path.'/modules/pagecomposer/media/js/js_head_'.$id_page.".js",file_get_contents($path.'/modules/pagecomposer/media/js/js_head_tmp_'.$id_page.".js"));
				file_put_contents($path.'/modules/pagecomposer/media/js/js_end_'.$id_page.".js",file_get_contents($path.'/modules/pagecomposer/media/js/js_end_tmp_'.$id_page.".js"));
				file_put_contents($path.'/modules/pagecomposer/media/css/css_'.$id_page.".css",file_get_contents($path.'/modules/pagecomposer/media/css/css_tmp_'.$id_page.".css"));
			...
			case 'get_widgets':
				...
				$module_widgets = $database->select('w.*,m.kind','widget as w join module as m on m.id=w.module',"m.active=1 AND (w.id_adv_page IS NULL OR w.id_adv_page={$id_page})");
				

				//PRENDO SOLO I WIDGET DELLA DASHBOARD
				if( $id_page == _PAGE_COMPOSER_DASHBOARD_PAGE_ID_ ){
			...
			case 'save_composition_box':
				...
				$k = 1;
				foreach($list as $v){
					$database->update('composition_page_tmp',"id={$v['id']}",array('orderView'=>$k));
					$k++;
			...
			case 'export_row':
				...
				if( !$id ){
					...
					$export = $database->select('*','composition_page_tmp',"id_adv_page={$id_page} order by orderView,id");
					
					/*$items = $database->select('*','composition_page_tmp',"parent=0 AND id_adv_page={$id_page} order by orderView,id");
					...
					$items = $database->select('*','composition_page_tmp',"id={$id} order by orderView,id");
					...
					if( okArray($items) ){
						...
						$items = $database->select('*','composition_page_tmp',"parent={$id} order by orderView,id");
						foreach($items as $v){
							//$v['parent'] = 0;
							$this->exportRow($v,$export);
						}
	...
	function getLastlogs(){
		$database = Marion::getDB();
		$id = _var('id');
		$list = $database->select('l.*,u.name,u.surname','pagecomposer_log as l join user as u on u.id=l.id_user',"id_page={$id} order by timestamp DESC LIMIT 1");
	...
	function importRow($id_page,$data,$id_parent=0){
		...
		$id_parent = $database->insert('composition_page_tmp',$data);
		if( okArray($children) ){
	...
	function exportRow($current,&$export){
		$export[] = $current;
		$database = Marion::getDB();
		$items = $database->select('*','composition_page_tmp',"parent={$current['id']} order by orderView,id");
		if( okArray($items) ){
			foreach($items as $v){
				$this->exportRow($v,$export);
			}
		...
		if( $this->isSubmitted()){
			...
			if( $array[0] == 'ok'){
				...
				if( $formcontrol == 'page_composer_row_conf'){
					$toupdate = $array;
					$database->update('composition_page_tmp',"id={$this->id_box}",$toupdate);
	...
	function clean($id){
		...
		$database = Marion::getDB();
		$list = $database->select('*','composition_page',"id_adv_page={$id}");
		$da_eliminare = array();
		$tot = 0;
		...
		while(count($parent) > 0 && $iter < 10){
			...
			if($where){
				$database->delete('composition_page',"id IN ({$where}) AND id_adv_page={$id}");
			}
