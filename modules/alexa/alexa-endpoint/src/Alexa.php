...
class Alexa
{
     ...
     */
    public static function ask($text, $repromt = "",$permissions=array())
    {
        $ssml = (strpos($text, '<speak>') !== false && strpos($text, '</speak>') !== false);
        $type = $ssml ? 'SSML' : 'PlainText';
        $property = $ssml ? 'ssml' : 'text';
		...
		if( count($permissions) > 0 ){
			$say->response->card->type = 'AskForPermissionsConsent';
			$say->response->card->permissions = $permissions;
		}
	...
	/**
     * Runs a function if Alexa skill message.
     * @param  callable $callback The callback function to run if Alexa just received message
     */
    public static function messages($callback) {
