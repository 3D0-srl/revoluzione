...
class AmazonSyncro{
	...
	//directories path 
	public $_dir_xml_feed_request = 'xml_upload';
	public $_dir_xml_feed_response = 'responses';
	public $_dir_report_response = 'reports';
	...
	function saveXMLRequest($name_file,$xml){
		if( !file_exists($this->_dir_xml_feed_request) ){
			mkdir($this->_dir_xml_feed_request);
		}
		file_put_contents($this->_dir_xml_feed_request.'/'.$name_file.".xml",$xml);
	}

	function saveXMLResponse($name_file,$xml){
		if( !file_exists($this->_dir_xml_feed_response) ){
			mkdir($this->_dir_xml_feed_response);
		}
			...
			if( okArray($res) ){
				...
				foreach($res as $v){
					if( $v['FeedProcessingStatus'] == '_DONE_' ){
						$this->getFeedResponse($v);
					}
					$database->update('amazon_feed_sync',"FeedSubmissionId='{$v['FeedSubmissionId']}'",array('FeedProcessingStatus'=>$v['FeedProcessingStatus']));
				}
			}
		}
	}
