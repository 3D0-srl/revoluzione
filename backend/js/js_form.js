	...
	if( typeof javascript_activelocale != 'undefined' && javascript_activelocale != null ){
			...
			tinymce.init({
				...
				file_browser_callback: function(field, url, type, win) {
						tinyMCE.activeEditor.windowManager.open({
							file: '/plugins/kcfinder-3.12/browse.php?opener=tinymce4&field=' + field + '&type=' + type +"&langCode="+javascript_activelocale,
							title: 'KCFinder',
							width: 700,
							height: 500,
							inline: true,
		...
		if ($('.mce-textarea-full').length ) {
			tinymce.init({
				...
				file_browser_callback: function(field, url, type, win) {
						tinyMCE.activeEditor.windowManager.open({
							file: '/plugins/kcfinder-3.12/browse.php?opener=tinymce4&field=' + field + '&type=' + type +"&langCode=it",
							title: 'KCFinder',
							width: 700,
							height: 500,
							inline: true,
	...
	if ($('.cke-editor-advanced').length && $.fn.ckeditor) {
	    $('.cke-editor-advanced').each(function () {
			$(this).ckeditor({
			   ...
			   filebrowserBrowseUrl : '/plugins/kcfinder-3.12/browse.php?opener=ckeditor&type=files',
			   filebrowserImageBrowseUrl : '/plugins/kcfinder-3.12/browse.php?opener=ckeditor&type=images',
			   filebrowserFlashBrowseUrl : '/plugins/kcfinder-3.12/browse.php?opener=ckeditor&type=flash',
			   filebrowserUploadUrl : '/plugins/kcfinder-3.12/upload.php?opener=ckeditor&type=files',
			   filebrowserImageUploadUrl : '/plugins/kcfinder-3.12/upload.php?opener=ckeditor&type=images',
			   filebrowserFlashUploadUrl : '/plugins/kcfinder-3.12/upload.php?opener=ckeditor&type=flash',
	...
	if( $('#img_filemanager_'+field_id).length > 0 ){
		...
		if( $('#'+field_id).val() ){
			...
			switch(extension) {
				...
				default:
					image = js_baseurl+'images/file-icons/512px/'+extension+".png";
					break;
			}
			if( type_image ){
				$('#img_filemanager_'+field_id).attr('src',js_baseurl+'media/images/'+image).show();
			}else{
