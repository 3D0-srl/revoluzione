...
class Form {
	 ...
	 /*public function getFilesInput($name=NULL){
		$elements = $this->getElements();
		if( $name ){
			$app = $elements;
			unset($elements);
			$elements[$name] = $app[$name];
		}
		foreach($elements as $k=>$v){
			if($v['type'] == 'file'){
				$extensions = '';
				if(okArray($v['extensions']) ){
					foreach($v['extensions'] as $v1){
						$extensions .= $v1.",";	
					}
					$extensions = preg_replace('/\,$/','',$extensions);
				}
				if( $v['dimension_resize_default'] ){
					$files[] = array(
						'name' => $v['campo'],
						'label' => $v['etichetta'],
						'type' => $v['tipo_file'],
						'number_files' => $v['number_files'],
						'extensions' => $extensions,
						'resize' => unserialize($v['resize_image'])
					);
					...
					$files[] = array(
						...
						'extensions' => $extensions,
						'resize' => unserialize($v['resize_image']),
						'dimensions_resize' => unserialize($v['dimension_image'])
					);
				}
		...
		foreach($form as $k => $v1){
				if( okArray($formdata) && array_key_exists($v1['campo'],$formdata) ){
					...
					/*if( $type == 'select' || $type == 'radio' || $type == 'checkbox' ||$type == 'multiselect' ){
						//determino i valori possibili
						if( isset($v1['origine_dati']) && $v1['origine_dati'] == 'php' && isset($v1['function_php']) ){
							$options = $this->generaDatiPHP($v1['function_php']);
						}else{
							$options = $v1['options'];
						}
					}*/
					...
					//controllo dei dati di tipo select
						...
						if( isset($v) ){
							/*debugga(array_keys($options));
							debugga($v);
							debugga(array_key_exists($v,$options));
							exit;*/
	 ...
	 /*public function getElements(){
		...
		if(okArray($campi)){
			...
			foreach( $campi as $c){
				...
				$c['checklunghezza'] = $this->trueFalse($c['checklunghezza']);
				$c['multilocale'] = $this->trueFalse($c['multilocale']);
				...
				if($c['post_function']){
					...
					if(okArray($list_post_function)){
						foreach($list_post_function as $function){
							if( function_exists($function)  || method_exists($template,$function) ){
								$c['post_function_array'][] = $function;
							}
						}
					}
				...
				if( $c['type'] == 'checkbox' ){
					$c['unique_value'] = $this->trueFalse($c['unique_value']);
				}else{
					unset($c['unique_value']);
				}
				...
				$c['tipo_valori'] = $this->trueFalse($c['tipo_valori']);
				$c['obbligatorio'] = $this->trueFalse($c['obbligatorio']); 
				$c['valuezero'] = $this->trueFalse($c['valuezero']); 
				if( $c['type'] == 'select' || $c['type'] == 'radio' || $c['type'] == 'checkbox' || $c['type'] == 'multiselect' ){
					if( $c['tipo_valori'] == 't'){
						...
						$template =_obj('Template');
						$function = $c['function_template'];
						if( $c['function_template']  && method_exists($template,$c['function_template']) ){
							if( $c['type'] == 'select' || $c['type'] == 'multiselect'){
								$c['options'] = $template->$function(); 
							}else{
								$c['options'] = array_keys($template->$function());
							} 	
