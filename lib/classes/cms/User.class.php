...
class User extends Base{
	...
	/*public static $livelli_auth = array(
		'1' 		=> "auth_base",
		'2' 		=> "auth_catalog", //permette di modificare, aggiungere o eliminare un prodotto
		//'4' 		=> "auth_cms", // permette di gestire i contenuti di un sito come le pagine etc.
		'8' 		=> "auth_newsletter", // permette di gestire la newsletter
		'16' 		=> "auth_config", //permette di configurare le impostazioni del sito
		'32' 		=> "auth_user_management", //permette di modificare,aggiungere,eliminare gli utenti
		'64' 		=> "auth_admin", // amministratore del sito (utente non modificabile)
		'128' 		=> "auth_ecommerce", // amministratore del sito (utente non modificabile)
		'256' 		=> "auth_superadmin", // super admin (accede al backend).
		'512' 		=> "auth_post", // permette di inserire dei post
		'1024' 		=> "auth_post_admin", // permette di gestire tutti i post
		'2048' 		=> "auth_cms", // permette di gestire i contenuti cms
		'4096' 		=> "auth_cms_page", // permette di gestire le pagine
		'8192'     => "auth_cms_gallery", // permette di gestire le gallery
		'16384'     => "auth_cms_news", // permette di gestire le gallery
	);*/
...
*/
	...
	public static function login($username_or_email,$password){
		if($username_or_email && $password ){
			...
			$query = $database->getQuerySelect('*',self::TABLE,"username=?");
			...
			if ( okArray($check) ){
				...
				$query = $database->getQuerySelect('*',self::TABLE,"email=?");
				$check = $database->prepare($query)
					->setParam($username_or_email,'string')
					->execute();
				$type = 'email';
	...
	/*public static function readPermissions(){
		$permissions = Permission::prepareQuery()->where('active',1)->get();
		foreach($permissions as $perm){
			self::$livelli_auth[$perm->code] = $perm->get('label');
		}

		//debugga(self::$livelli_auth);exit;
	}*/
	...
	/*function getPermissions(){
		$this->auth_livelli = array();

		
		if( $this->id_profile ){
			$profile = Profile::withId($this->id_profile);
			if( is_object($profile) ){
				$this->auth_livelli = $profile->permissions;
			}
		}
		$this->auth_livelli[] = 1;
		foreach($this->auth_livelli as $v){
			$this->auth += $v;	
		}
		sort($this->auth_livelli);
	...
	*/
	...
	//aggiunge un livello di permessi all'utente
	/*function addPermission($livello){
		
		if( array_key_exists($livello,self::$livelli_auth) && !in_array($livello,$this->auth_livelli)){ 
			foreach($this->auth_livelli as $v){
				$livello += $v;
			}
			$this->auth = $livello;
	...
	//aggiunge un array di livelli di permessi all'utente
	function addPermissionArray($array){
		if(okArray($array)){
			foreach($array as $v){
	...
	//aggiunge tutti i livelli di permessi all'utente
	function addAllPermissions(){
		foreach( array_keys(self::$livelli_auth) as $v ){
			$this->addPermission($v);	
		}
		return $this;
	}
	...
	//elimina un livello di permessi dell'utente
	function deletePermission($livello){
		if( array_key_exists($livello,self::$livelli_auth) && in_array($livello,$this->auth_livelli)){ 
			$livello_new = 0;
	...
	//rimuove tutti i permessi dell'utente
	function deleteAllPermissions(){
		foreach( array_keys(self::$livelli_auth) as $v ){
			$this->deletePermission($v);	
		}
	...
	//funzioni di autorizzazione generica
	/*function auth($livello){
		self::readPermissions();
		
		$livello = array_search($livello,self::$livelli_auth);
	...
	function auth($livello){
		...
		//return $this->checkPermission($livello);
	}
	...
	public function prepareForm($locale='all'){
		...
		//$data['permissions'] = $this->auth_livelli;
		return $data;
