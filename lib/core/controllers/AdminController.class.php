...
require_once(_MARION_LIB_."core/elements/ListActionBulkButton.class.php");
require_once(_MARION_LIB_."core/elements/ListActionRowButton.class.php");
require_once(_MARION_LIB_."core/elements/ListContainer.class.php");
class AdminController extends Controller{
	...
	private $_form_container; //ogetto form container


	/* LIST ATTRIBUTES */
	public $_limit_list = 25;
	public $_list_options = array(
		'html_template' => 'common/list.htm',
	...
	//VECCHIO PAGER DA RIMUOVERE
	function buildPagerList(){

		
		$params = array();
		$params['totalItems'] = $this->_list_options['total_items'];
		$params['mode'] = 'Sliding';
		$params['perPage'] = $this->_list_options['per_page'];
		$params['linkClass'] = 'pager-admin';
		$params['separator'] = '';
		$params['spacesBeforeSeparator'] = 1;
		$params['spacesAfterSeparator'] = 1;
		$params['curPageLinkClassName'] = "pager-admin-current";
	...
	function setFormContainer($form){
		$this->_form_container = $form;
		return $this;
	}
	...
	/*function getCtrl(){
		...
		return preg_replace('/Controller/','',$name_class);
		
	}*/

	//restituisce il pager della lista a partire dal totale degli elementi che ne fanno parte
	function getPagerList($tot){
		$params['totalItems'] = $tot;
		$params['mode'] = 'Sliding';
		$params['perPage'] = $this->getLimitList();
		$params['linkClass'] = 'pager-admin';
		$params['separator'] = '';
		$params['spacesBeforeSeparator'] = 1;
		$params['spacesAfterSeparator'] = 1;
	...
	//Stabilisce se un utente è abilitato o meno ad accedere ad una pagina
	function checkAccess(){
		return $this->_user->auth($this->_auth);
	}
	//Stabilisce se un utente è loggato o meno
	function isLogged(){
		return authUser();
	}*/
	...
	/*function output($tmpl,$elements){
		$this->setTemplateVariables();
		$this->displayErrors();
		$this->_tmpl_obj->output($tmpl,$elements);
	}

	// imposta una variabile nel template
	function setVar($key,$val){
		$this->_tmpl_obj->$key = $val;
	}*/
	...
	function displayList(){
		...
		$this->buildList();
		if( _var('export') ){
			$filename=$this->_list_options['title'];
			switch(_var('type_export')){
				...
				case 'pdf':
					...
					ob_start();
					$this->output('common/list_pdf.htm');
					$html = ob_get_contents();
					ob_end_clean();
					ob_flush();
	...
	function displayForm(){
	   ...
	   if($this->_form_container){
			$this->_form_container->build();
			
			$html = $this->_form_container->getHtml();
	...
	function confirmDelete(){
		...
		$this->output('common/confirm_delete.htm');
	}

	function getConfirmDeleteTitle(){
		return _translate('confirm_delete');
	}

	function getConfirmDeleteMessage(){
		return _translate('confirm_delete_message');
	}
