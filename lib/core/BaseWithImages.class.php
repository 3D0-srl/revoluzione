...
class BaseWithImages extends Base{
	...
	public function getUrlImagesArray($type='original',$watermark=true){
		...
		if(okArray($this->$field_images) ){
			...
			foreach( $this->$field_images as $v){
				$image = $database->select('t2.path',"imageComposed as t1 join image as t2 on t1.{$type}=t2.id","t1.id={$v}");
				if(okArray($image)){
					$toreturn[] = STATIC::BASE_PATH.$image[0]['path'];
				}
	...
	function getAbsolutePathImage($index=0,$type='original'){
		...
		if(okArray($idImages) ){
			...
			if(okArray($img)){
				return  $_SERVER['DOCUMENT_ROOT']."/".$img[0]['path'];
